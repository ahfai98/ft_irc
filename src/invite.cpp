...
void Server::INVITE(const std::string &cmd, int fd)
{
    ...
    {
        sendResponse(fd, ":ircserv 403 " + inviterNick + " " + channelName + " :No such channel\r\n");
        return;
    }
    ...
    {
        sendResponse(fd, ":ircserv 442 " + inviterNick + " " + channelName + " :You're not on that channel\r\n");
        return;
    }
    if (ch->getInviteMode() && !ch->isChannelOperator(inviterNick))
    {
        sendResponse(fd, ":ircserv 482 " + inviterNick + " " + channelName + " :You're not channel operator\r\n");
        return;
    }
    if (ch->isInChannel(targetNick))
    {
        sendResponse(fd, ":ircserv 443 " + inviterNick + " " + targetNick + " " + channelName + " :is already on channel\r\n");
        return;
    }
    ch->addInvited(targetNick);
    // Notify inviter
    sendResponse(fd, ":ircserv 341 " + inviterNick + " " + targetNick + " " + channelName + "\r\n");
    // Notify target
